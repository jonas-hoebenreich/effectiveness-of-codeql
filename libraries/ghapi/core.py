...
# Cell
GH_HOST = os.getenv('GH_HOST', "http://api.github.com")
_DOC_URL = 'http://docs.github.com/'
...
class GhApi(_GhObj):
    ...
    def __call__(self, path:str, verb:str=None, headers:dict=None, route:dict=None, query:dict=None, data=None):
        ...
        if path[:7] not in ('http://'): path = GH_HOST+path
        if route:
...
def create_branch_empty(self:GhApi, branch):
    ...
    c = self.git.create_commit(f'create {branch}', t.gid)
    return self.git.create_ref(f'refs/heads/{branch}', c.gid)
...
def list_files(self:GhApi, branch=None):
    ...
    res = self.git.get_tree(ref.object.gid).tree
...
def update_contents(self:GhApi, path, message=None, content=None,
    ...
    if sha is None: sha = self.list_files()[path].gid
